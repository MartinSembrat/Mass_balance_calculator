...
public interface ISaleRepository extends JpaRepository<Sale, Integer> {
    @Query(value = "SELECT * FROM SALES where (JM='KG')", nativeQuery = true)
    List<Sale> findSaleByJM();

    @Query(value = "SELECT Sales.indeks as indeks, Sum(Sales.ilosc) AS sumaOfIlosc FROM Sales GROUP BY Sales.indeks", nativeQuery = true)
    List<IRMContentInIndex> findIRMContentInIndex();
    ...
    @Query(value = "SELECT tbl_wyroby.ID_wyrobu, tbl_surowce.Index, tbl_wyroby.Nazwa_wyrobu, tbl_surowce.Index AS Receptura, tbl_wyroby_sklad.Naważka AS Amount_for_primary, tbl_wyroby.INDEKS_CMJ\n" +
            ...
            "WHERE (((tbl_surowce.Index)='34A03004') AND ((tbl_wyroby_sklad.ID_typskladnika)=1))", nativeQuery = true)
    List<IRMContentInFG> findIRMContentInFG();
...
@Query(value = "SELECT tbl_wyroby.ID_wyrobu, tbl_surowce.Index, tbl_wyroby.Nazwa_wyrobu, tbl_ciasto_calosc.Nr_receptury AS Receptura, CAST(tbl_ciasto_calosc.Ilosc_surowca AS REAL)* tbl_wyroby_sklad.Naważka/qry_Wagi_mixu_ciasto .Waga_Mixu AS Amount_for_primary, tbl_wyroby.Indeks_CMJ\n" +
        ...
        "WHERE (((tbl_surowce.Index)='34A03004') AND ((tbl_wyroby.Indeks_CMJ) Is Not Null) AND ((tbl_wyroby_sklad.ID_typskladnika)=2) AND ((tbl_ciasto_calosc.Wersja)=tbl_ciasto.Wersja) AND ((tbl_ciasto.Aktualna)=1) AND ((qry_Wagi_mixu_ciasto.Wersja)=tbl_ciasto.Wersja));", nativeQuery = true)
List<IRMContentInFG> findIRMContentInFGCakes();
    ...
    @Query(value = "SELECT tbl_wyroby.ID_wyrobu, tbl_surowce.Index, tbl_wyroby.Nazwa_wyrobu, \n" +
            ...
            "WHERE (((tbl_surowce.Index)='34P00200') AND ((tbl_wyroby.Indeks_CMJ) Is Not Null) AND ((tbl_wyroby_sklad.ID_typskladnika)=2) AND ((tbl_krem_calosc.Wersja)=tbl_kremy.Wersja) AND ((tbl_kremy.Aktualna)=1) AND ((qry_Wagi_mixu_krem.Wersja)=tbl_kremy.Wersja));", nativeQuery = true)
    List<IRMContentInFG> findIRMContentInFGFillings();
